...
/*Constants*/
define('AJS_FOOTNOTES_VERSION','1.1');
...
class ajs_footnotes {
	...
	private function create_popups($number) {
		...
		$container_open = '<'.$this->options['popup_container_element'].' id="'.$this->options['id_lead'].'_'.($number+1).'-'.$post->ID.'" style="display:none;margin:0;" class="ajs-footnote-popup">';
	...
	private function remove_note_content($matches) {
		...
		//Convert (if needed)
		$display_value = $this->convert_numeral($this->match_count);
	...
	public function q_scripts() {
		wp_enqueue_script(
			'hoverIntent',
			plugins_url('hoverIntent.js', __FILE__),
			array('jquery')
		);
	}
	/**
	...
	jQuery(function($){
		...
		$('.ajs-footnote>a').hoverIntent({
			over: function(event) {
				...
				var popupWidth = $('<?php $this->options['id_lead'];?>_'+noteNumber).width();
	...
	public function insertCSS() {
		...
		if( $this->options['display_popups']) { ?>
			...
			<?php echo $this->options['popup_container_element'];?>.ajs-footnote-popup <?php echo $this->options['popup_container_element'];?> {
				...
				width: <?php echo $this->options['popup_width'].$this->options['popup_width_unit'];?>;
	...
	public function enqueueAdminScripts() {
		wp_register_script('colorPicker', plugins_url('js/jquery.minicolors.js', __FILE__), 'jquery');
